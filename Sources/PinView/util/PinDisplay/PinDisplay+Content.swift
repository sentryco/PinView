...
extension PinDisplay {
    ...
    * - Fixme: ⚠️️ Use spacing const from constlib? or add spacing variable as a struct variable?
    */
   public var body: some View {
      HStack(alignment: .center, spacing: 15) { // Creates a horizontal stack with center alignment and 15 spacing
         ForEach(str.map { $0 }, id: \.self) { (_ char: Character) in // Directly iterate over each character in the string
