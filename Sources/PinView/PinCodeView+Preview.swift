...
/**
 * Preview (Dark-mode)
 * - Description: Provides a preview of the `PinCodeView` in dark mode, showcasing the user interface for pin code input. The preview includes a `PinCodeView` configured with a 4-digit count and a sample verification callback that prints the entered pin and provides haptic feedback on iOS devices.
